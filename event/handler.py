...
from api import services as api_svc
...
class EventConsumer(bootsteps.ConsumerStep):
    ...
    def event_dispatcher(self, event, payload):
        ...
        event_func = getattr(self.api_svc, event.replace(".", "_"), self.no_event_func)
        event_func(payload, event)

    def no_event_func(self, payload, event):
        logger.error(f"unknown event [{event}] [{payload['actor']}]")
